...
void VulkanScene::initialize(vk::PhysicalDevice physicalDevice, vk::Instance instance, vk::Device device, vk::CommandPool commandPool, vk::Queue queue, unsigned queueIndex)
{
    _device = device;
    ...
    _immediateSubmit.initialize(_device, _queue, _queueIndex);
}
...
void VulkanScene::setScene(std::shared_ptr<Scene> scene, vk::RenderPass renderPass, vkb::Swapchain swapchain, unsigned int framesInFlight)
{
    ...
    //_GLTFPBRMaterial.free();
    _GLTFPBRMaterial.initialize(_device, _allocator, _renderPass, swapchain);
...
{
    VulkanDrawableCollection pbrCollection = {};
    pbrCollection.materialPipeline = _GLTFPBRMaterial.getMaterialPipeline();
    _collectionMap[MaterialName::GLTFPBR] = pbrCollection;
    ...
    for (auto& collection : _collectionMap)
    {
        collection.second.indices.buffer.create(_allocator, collection.second.indices.count * sizeof(uint32_t), vk::BufferUsageFlagBits::eTransferDst | vk::BufferUsageFlagBits::eIndexBuffer, VMA_MEMORY_USAGE_GPU_ONLY);
        collection.second.indices.stagingBuffer.create(_allocator, collection.second.indices.count * sizeof(uint32_t), vk::BufferUsageFlagBits::eTransferSrc, VMA_MEMORY_USAGE_CPU_ONLY);
        ...
        {
            collection.second.positions.buffer.create(_allocator, collection.second.positions.count * 4 * sizeof(float), bufferUsage, VMA_MEMORY_USAGE_GPU_ONLY);
            collection.second.positions.stagingBuffer.create(_allocator, collection.second.positions.count * 4 * sizeof(float), vk::BufferUsageFlagBits::eTransferSrc, VMA_MEMORY_USAGE_CPU_ONLY);
        ...
        {
            collection.second.normals.buffer.create(_allocator, collection.second.normals.count * 4 * sizeof(float), bufferUsage, VMA_MEMORY_USAGE_GPU_ONLY);
            collection.second.normals.stagingBuffer.create(_allocator, collection.second.normals.count * 4 * sizeof(float), vk::BufferUsageFlagBits::eTransferSrc, VMA_MEMORY_USAGE_CPU_ONLY);
        ...
        {
            collection.second.tangents.buffer.create(_allocator, collection.second.tangents.count * 4 * sizeof(float), bufferUsage, VMA_MEMORY_USAGE_GPU_ONLY);
            collection.second.tangents.stagingBuffer.create(_allocator, collection.second.tangents.count * 4 * sizeof(float), vk::BufferUsageFlagBits::eTransferSrc, VMA_MEMORY_USAGE_CPU_ONLY);
        ...
        {
            collection.second.texcoords_0.buffer.create(_allocator, collection.second.texcoords_0.count * 2 * sizeof(float), bufferUsage, VMA_MEMORY_USAGE_GPU_ONLY);
            collection.second.texcoords_0.stagingBuffer.create(_allocator, collection.second.texcoords_0.count * 2 * sizeof(float), vk::BufferUsageFlagBits::eTransferSrc, VMA_MEMORY_USAGE_CPU_ONLY);
        ...
        {
            collection.second.colors_0.buffer.create(_allocator, collection.second.colors_0.count * 4 * sizeof(float), bufferUsage, VMA_MEMORY_USAGE_GPU_ONLY);
            collection.second.colors_0.stagingBuffer.create(_allocator, collection.second.colors_0.count * 4 * sizeof(float), vk::BufferUsageFlagBits::eTransferSrc, VMA_MEMORY_USAGE_CPU_ONLY);
...
void VulkanScene::createDrawablesFromScene(std::shared_ptr<Scene> scene, std::shared_ptr<Node> rootNode)
{
    if (rootNode->mesh)
    {
        for (const auto& primitive : rootNode->mesh->primitives)
        {
            ...
            {
                drawable.firstIndex = 0; // TODO: this needs to be set properly, requires a seperate field in collection probably
                ...
                drawable.addresses.positions = collection.positions.bufferAddress + collection.positions.dataOffset;
                drawable.addresses.normals = collection.normals.bufferAddress + collection.normals.dataOffset;
                drawable.addresses.tangents = collection.tangents.bufferAddress + collection.tangents.dataOffset;
                drawable.addresses.texcoords_0 = collection.texcoords_0.bufferAddress + collection.texcoords_0.dataOffset;
                drawable.addresses.colors_0 = collection.colors_0.bufferAddress + collection.colors_0.dataOffset;
...
void VulkanScene::loadPrimitiveAttributeDataToBuffer(VulkanDrawableCollection& collection, std::shared_ptr<Primitive> primitive)
{
...
void VulkanScene::copyStagingBufferDataToBuffers(VulkanDrawableCollection& collection)
{
    ...
    _immediateSubmit.submit([&](vk::CommandBuffer buffer)
        {
            buffer.copyBuffer(collection.indices.stagingBuffer.buffer, collection.indices.buffer.buffer, 1, &indicesCopyRegion);
        });
    _queue.waitIdle();
    ...
    if (collection.positions.buffer)
    {
        ...
        _immediateSubmit.submit([&](vk::CommandBuffer buffer)
            {
                buffer.copyBuffer(collection.positions.stagingBuffer.buffer, collection.positions.buffer.buffer, 1, &positionsCopyRegion);
            });
        _queue.waitIdle();
    }
    if (collection.normals.buffer)
    {
        ...
        _immediateSubmit.submit([&](vk::CommandBuffer buffer)
            {
                buffer.copyBuffer(collection.normals.stagingBuffer.buffer, collection.normals.buffer.buffer, 1, &normalsCopyRegion);
            });
        _queue.waitIdle();
    }
    if (collection.tangents.buffer)
    {
        ...
        _immediateSubmit.submit([&](vk::CommandBuffer buffer)
            {
                buffer.copyBuffer(collection.tangents.stagingBuffer.buffer, collection.tangents.buffer.buffer, 1, &tangentsCopyRegion);
            });
        _queue.waitIdle();
    }
    if (collection.texcoords_0.buffer)
    {
        ...
        _immediateSubmit.submit([&](vk::CommandBuffer buffer)
            {
                buffer.copyBuffer(collection.texcoords_0.stagingBuffer.buffer, collection.texcoords_0.buffer.buffer, 1, &texcoords_0CopyRegion);
            });
        _queue.waitIdle();
    }
    if (collection.colors_0.buffer)
    {
        ...
        _immediateSubmit.submit([&](vk::CommandBuffer buffer)
            {
                buffer.copyBuffer(collection.colors_0.stagingBuffer.buffer, collection.colors_0.buffer.buffer, 1, &colors_0CopyRegion);
            });
        _queue.waitIdle();
    }
}
