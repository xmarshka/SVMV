...
void main() {
    ...
    gl_Position = cam_mats.view_proj_mat * pc.model_mat.data[0] * vec4(ls_P, 1.0);
    out_ws_P = vec3(pc.model_mat.data[0] * vec4(ls_P, 1.0));
    ...
    if (uvec2(pc.N_buf) != uvec2(0)){
        mat3 normal_mat = mat3(transpose(inverse(pc.model_mat.data[0])));
